...
import {
  User, Building2, Landmark, ChevronRight, ChevronLeft,
  CheckCircle2, Loader2, ArrowLeft, MapPin, Package,
  CreditCard, Users, BookOpen, Monitor, GraduationCap,
  Wrench, Wifi, Phone,
} from "lucide-react";
...
const ADDITIONAL_NEEDS = [
  { id: "ar_books",             label: "AR Books",             Icon: BookOpen },
  { id: "teacher_training",     label: "Teacher Training",     Icon: GraduationCap },
  { id: "school_demo",          label: "School Demo",          Icon: Monitor },
  { id: "installation_support", label: "Installation Support", Icon: Wrench },
  { id: "lms_access",           label: "LMS Access",           Icon: Wifi },
];

const USER_TYPES = [
  { id: "individual", label: "Parent / Individual", Icon: User },
  { id: "school",     label: "School",              Icon: Building2 },
  { id: "institution",label: "Government / Institution", Icon: Landmark },
];

const PAYMENT_OPTIONS = [
  { id: "full",        label: "Full Payment",        desc: "Pay the full amount upfront." },
  { id: "installment", label: "Installment",          desc: "Spread payments over time." },
  { id: "discuss",     label: "Discuss With Team",   desc: "We'll reach out to arrange." },
];
...
export default function PreorderPage() {
  ...
  const validate = () => {
    ...
    if (step === 5) {
      if (!form.payment_option)    e.payment_option    = "Please select a payment option.";
      if (!form.agreed_to_contact) e.agreed_to_contact = "You must agree to be contacted.";
    }
    setErrors(e);
  ...
  const isSchoolOrInstitution = ["school", "institution"].includes(form.user_type);
      ...
      <div className="min-h-screen bg-[#f0f9ff] flex items-center justify-center px-4 py-20">
        ...
        >
          ...
          <p className="text-gray-500 text-sm leading-relaxed mb-8">
            Thank you, {form.full_name.split(" ")[0]}! Your reservation has been logged. Our team will contact you within 24 hours to confirm your slot.
          </p>
          <Link
      ...
      <div className="bg-secondary py-12 px-4 relative overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto relative">
          ...
          <p className="text-white/65 text-sm sm:text-base">
            Nigeria&apos;s immersive STEM learning experience for children ages 5–11.
          </p>
        </div>
      </div>
          ...
          >
            ...
            {step === 1 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-secondary mb-1" style={{ fontFamily: "var(--font-jarkata)" }}>I Am A…</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {USER_TYPES.map(({ id, label, Icon }) => (
                    ...
                    >
                      ...
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${form.user_type === id ? "bg-primary text-white" : "bg-gray-100 text-gray-500"}`}>
                        <Icon className="w-6 h-6" strokeWidth={1.75} />
                      </div>
                      <span className={`text-sm font-bold ${form.user_type === id ? "text-primary" : "text-secondary"}`}>{label}</span>
                    </button>
                  ))}
                </div>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-secondary mb-1" style={{ fontFamily: "var(--font-jarkata)" }}>Your Details</h2>
                  <p className="text-gray-500 text-sm">We&apos;ll use these to reach you about your order.</p>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
                  <div>
                    <Label required>Full Name</Label>
                    <Input placeholder="e.g. Chukwuemeka Obi" value={form.full_name} onChange={(e) => set("full_name", e.target.value)} />
                    <FieldError msg={errors.full_name} />
                  </div>
                  {isSchoolOrInstitution && (
                    <div>
                      <Label>School / Organisation Name</Label>
                      <Input placeholder="e.g. Greenfield Academy" value={form.school_org_name} onChange={(e) => set("school_org_name", e.target.value)} />
                    </div>
                  )}
                  <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input className="pl-9" type="tel" placeholder="+234 801 234 5678" value={form.whatsapp} onChange={(e) => set("whatsapp", e.target.value)} />
                      </div>
            ...
            {step === 3 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Helps us plan delivery and support in your area.</p>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
                  <div>
            ...
            {step === 4 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-secondary mb-1" style={{ fontFamily: "var(--font-jarkata)" }}>Order Details</h2>
                  <p className="text-gray-500 text-sm">Tell us how many devices and what support you need.</p>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => set("device_count", parseInt(e.target.value, 10) || "")}
                    />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {ADDITIONAL_NEEDS.map(({ id, label, Icon }) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${checked ? "bg-primary text-white" : "bg-gray-100 text-gray-400"}`}>
                              <Icon className="w-4 h-4" strokeWidth={1.75} />
                            </div>
                            <span className={`text-sm font-semibold ${checked ? "text-primary" : "text-gray-600"}`}>{label}</span>
                            {checked && <CheckCircle2 className="w-4 h-4 text-primary ml-auto shrink-0" />}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {isSchoolOrInstitution && (
                    <div className="pt-2 border-t border-gray-100">
                      <p className="text-xs font-bold text-primary uppercase tracking-wider mb-4">For Schools / Institutions</p>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label>Number of Students</Label>
                          <Input type="number" min={0} placeholder="e.g. 200" value={form.student_count} onChange={(e) => set("student_count", e.target.value)} />
                        </div>
                        <div>
                          <Label>Number of Teachers</Label>
                          <Input type="number" min={0} placeholder="e.g. 15" value={form.teacher_count} onChange={(e) => set("teacher_count", e.target.value)} />
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {step === 5 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-secondary mb-1" style={{ fontFamily: "var(--font-jarkata)" }}>Payment Preference</h2>
                  <p className="text-gray-500 text-sm">Choose how you&apos;d like to handle payment.</p>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
                  <div className="space-y-3">
                    {PAYMENT_OPTIONS.map(({ id, label, desc }) => (
                      ...
                      >
                        <div className={`w-5 h-5 rounded-full border-2 shrink-0 mt-0.5 flex items-center justify-center ${form.payment_option === id ? "border-primary bg-primary" : "border-gray-300"}`}>
                          {form.payment_option === id && <div className="w-2 h-2 rounded-full bg-white" />}
                        </div>
                        <div>
                          <p className={`text-sm font-bold ${form.payment_option === id ? "text-primary" : "text-secondary"}`}>{label}</p>
                  ...
                  <div className="pt-3 border-t border-gray-100">
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <div
                        onClick={() => set("agreed_to_contact", !form.agreed_to_contact)}
                      ...
                      >
                        {form.agreed_to_contact && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                      </div>
                      <span className="text-sm text-gray-600 leading-relaxed">
                        I agree to be contacted by Blue Sands STEM Labs regarding my preorder.
                      </span>
                    </label>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                    <p className="font-bold text-secondary text-xs uppercase tracking-wider mb-2">Order Summary</p>
                    <div className="flex justify-between text-gray-600"><span>Type</span><span className="font-semibold capitalize">{form.user_type}</span></div>
                    <div className="flex justify-between text-gray-600"><span>Devices</span><span className="font-semibold">{form.device_count}</span></div>
                    <div className="flex justify-between text-gray-600"><span>State</span><span className="font-semibold">{form.state}</span></div>
                  </div>
