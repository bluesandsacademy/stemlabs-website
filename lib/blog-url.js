...
export function buildBlogHref(current = {}, overrides = {}) {
  ...
  // If we're overriding any filter parameter (not page), reset page to 1
  // unless the overrides explicitly set page
  const isOverridingFilter = Object.keys(overrides).some(key =>
    key !== 'page'
  );
  if (isOverridingFilter && !('page' in overrides)) {
    delete merged.page;
  }
